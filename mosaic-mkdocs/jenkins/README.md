...
Configure the Job to use a `Pipeline script from SCM` with the target SCM being `Git`. Use the repository URL `git.cwp.pnp-hcl.com:CWPdoc/total-experience-mkdocs.git` and a fitting git credential, e.g. `git-ssh-access-key`.
