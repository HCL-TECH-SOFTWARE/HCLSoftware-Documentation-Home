...
{% if isLeap %}
- Added [My Tasks](sub_adding_stages_toc.md#my-tasks) page
- Added an extension point for a server-side data validator
{% endif %}
- Added Swagger UI to interact with REST APIs
- Updated REST API definitions to OpenAPI v3
- Added OpenApi definitions for {% if isLeap %}[My Tasks](ref_my_tasks_rest_api.md) and {% endif %} [Application Statistics](app_stats_restapi.md) REST APIs
- Members can now be searched using [custom lookup properties](co_configuration_properties.md#customuserlookupproperties)
